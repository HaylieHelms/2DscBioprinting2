...
# Output folder and workbook path
source_dir <- "Nature_Source_Data"
dir.create(source_dir, showWarnings = FALSE, recursive = TRUE)

fig4_source_file <- file.path(source_dir, "Figure4_SourceData.xlsx")

# -----------------------------
# Workbook helpers
# -----------------------------

load_or_create_wb <- function(path = fig4_source_file) {
  if (file.exists(path)) {
...
add_or_replace_sheet <- function(wb, sheet, data) {
  ...
  openxlsx::writeData(wb, sheet, as.data.frame(data))
  openxlsx::freezePane(wb, sheet, firstRow = TRUE)

  if (ncol(as.data.frame(data)) > 0) {
    openxlsx::setColWidths(
      wb,
      sheet,
      cols = 1:ncol(as.data.frame(data)),
      widths = "auto"
    )
  }
...
arrange_existing <- function(data, cols) {
  cols <- intersect(cols, names(data))
  if (length(cols) == 0) return(data)
  data %>% dplyr::arrange(dplyr::across(dplyr::all_of(cols)))
}
...
default_covariates <- "No additional covariates were tested; analyses compared predefined experimental groups."
default_hierarchy  <- "Measurements may be nested within independent bioprinted arrays and/or repeated over time; the analysis level and experimental unit are stated for each statistics table."

make_summary <- function(data, group_cols, value_col,
                         units = NA_character_,
                         n_unit = "observations") {
  data %>%
...
eta_squared_manual <- function(data, value_col, group_col) {
  d <- data %>%
    dplyr::filter(is.finite(.data[[value_col]]), !is.na(.data[[group_col]]))

  if (nrow(d) < 2 || dplyr::n_distinct(d[[group_col]]) < 2) {
    return(NA_real_)
  }

  grand_mean <- mean(d[[value_col]], na.rm = TRUE)

  ss_total <- sum((d[[value_col]] - grand_mean)^2, na.rm = TRUE)
  ss_between <- d %>%
    ...
    dplyr::summarise(ss_between = sum(n * (group_mean - grand_mean)^2, na.rm = TRUE)) %>%
    dplyr::pull(ss_between)

  if (!is.finite(ss_total) || ss_total == 0) return(NA_real_)
  ss_between / ss_total
}

hedges_g_manual <- function(data, value_col, group_col, group1, group2) {
  d <- data %>%
    dplyr::filter(.data[[group_col]] %in% c(group1, group2)) %>%
    dplyr::filter(is.finite(.data[[value_col]]), !is.na(.data[[group_col]]))

  x1 <- d[[value_col]][d[[group_col]] == group1]
  x2 <- d[[value_col]][d[[group_col]] == group2]
  n1 <- length(x1)
  n2 <- length(x2)
  ...
  if (n1 < 2 || n2 < 2) {
    return(tibble::tibble(
      ...
      effect_size_calculation = "Hedges' g calculated as bias-corrected Cohen's d using pooled standard deviation."
    ))
  }
  ...
  cohen_d <- (mean(x1, na.rm = TRUE) - mean(x2, na.rm = TRUE)) / pooled_sd
  correction <- 1 - (3 / (4 * (n1 + n2) - 9))
  hedges_g <- correction * cohen_d

  # Approximate large-sample confidence interval for standardized mean difference.
  se_d <- sqrt((n1 + n2) / (n1 * n2) + (cohen_d^2 / (2 * (n1 + n2 - 2))))
  se_g <- correction * se_d

  tibble::tibble(
    effect_size_type = "Hedges_g",
    effect_size = hedges_g,
    effect_size_ci_low = hedges_g - 1.96 * se_g,
    effect_size_ci_high = hedges_g + 1.96 * se_g,
    effect_size_calculation = "Hedges' g calculated as bias-corrected Cohen's d using pooled standard deviation; CI is an approximate normal-theory interval."
  )
}

make_stats_context <- function(n_rows,
                               sided,
                               covariates_tested = default_covariates,
                               assumptions_or_corrections,
                               experimental_unit,
                               analysis_level,
                               hierarchical_level = default_hierarchy) {
  tibble::tibble(
    sided = rep(sided, n_rows),
    covariates_tested = rep(covariates_tested, n_rows),
    assumptions_or_corrections = rep(assumptions_or_corrections, n_rows),
    experimental_unit = rep(experimental_unit, n_rows),
    analysis_level = rep(analysis_level, n_rows),
    hierarchical_level = rep(hierarchical_level, n_rows)
  )
}
...
make_anova_tukey_table <- function(data, value_col, group_col, split_col = NULL,
                                   metric = value_col,
                                   experimental_unit = "independent bioprinted array",
                                   analysis_level = "tracked cell trajectory",
                                   hierarchical_level = "Tracked cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized per trajectory before testing.",
                                   assumptions_or_corrections = "One-way ANOVA was used for predefined group comparisons; Tukey adjustment was used for multiple pairwise comparisons.",
                                   covariates_tested = default_covariates) {
  ...
  split_list <- if (!is.null(split_col)) {
    data0 %>%
      dplyr::group_by(dplyr::across(dplyr::all_of(split_col))) %>%
      dplyr::group_split(.keep = TRUE)
  } else {
    list(data0)
  }

  dplyr::bind_rows(lapply(split_list, function(d) {
    ...
    analysis_group <- if (!is.null(split_col)) {
      paste(unique(as.character(d[[split_col]])), collapse = "; ")
    } else {
      NA_character_
    }

    if (dplyr::n_distinct(d[[group_col]]) < 2) {
      return(tibble::tibble())
    }

    fit <- stats::aov(stats::reformulate(group_col, response = value_col), data = d)
    tidy_fit <- broom::tidy(fit)

    global_term <- tidy_fit %>%
      dplyr::filter(term == group_col)

    residual_term <- tidy_fit %>%
      dplyr::filter(term == "Residuals")

    eta2 <- eta_squared_manual(d, value_col, group_col)

    anova_row <- tibble::tibble(
      metric = metric,
      analysis_group = analysis_group,
      comparison = "Global test",
      test = "One-way ANOVA",
      statistic = "F",
      statistic_value = global_term$statistic[1],
      df1 = global_term$df[1],
      df2 = residual_term$df[1],
      estimate = NA_real_,
      estimate_type = NA_character_,
      ci_low = NA_real_,
      ci_high = NA_real_,
      p_value = global_term$p.value[1],
      p_adjust_method = "none",
      p_adjusted = NA_real_,
      significance = p_to_stars(global_term$p.value[1]),
      effect_size_type = "eta_squared",
      effect_size = eta2,
      effect_size_ci_low = NA_real_,
      effect_size_ci_high = NA_real_,
      effect_size_calculation = "Eta-squared calculated as between-group sum of squares divided by total sum of squares.",
      n_total = nrow(d),
      n_group1 = NA_integer_,
      n_group2 = NA_integer_
    ) %>%
      dplyr::bind_cols(make_stats_context(
        n_rows = 1,
        sided = "not applicable; omnibus F test",
        covariates_tested = covariates_tested,
        assumptions_or_corrections = assumptions_or_corrections,
        experimental_unit = experimental_unit,
        analysis_level = analysis_level,
        hierarchical_level = hierarchical_level
      ))

    tukey_raw <- TukeyHSD(fit, group_col)[[group_col]] %>%
      as.data.frame() %>%
    ...
    names(group_counts)[1] <- "group_name"

    tukey_effects <- dplyr::bind_rows(lapply(seq_len(nrow(tukey_raw)), function(i) {
      hedges_g_manual(
        data = d,
        value_col = value_col,
        group_col = group_col,
        group1 = tukey_raw$group1[i],
        group2 = tukey_raw$group2[i]
      )
    }))

    tukey_rows_core <- dplyr::bind_cols(tukey_raw, tukey_effects) %>%
      dplyr::mutate(
        metric = metric,
        analysis_group = analysis_group,
        ...
        significance = p_to_stars(p_adjusted),
        n_total = nrow(d),
        n_group1 = group_counts$n[match(group1, group_counts$group_name)],
        n_group2 = group_counts$n[match(group2, group_counts$group_name)]
      ) %>%
      dplyr::select(
        metric, analysis_group, comparison, test, statistic, statistic_value,
        df1, df2, estimate, estimate_type, ci_low, ci_high,
        p_value, p_adjust_method, p_adjusted, significance,
        effect_size_type, effect_size, effect_size_ci_low, effect_size_ci_high,
        effect_size_calculation, n_total, n_group1, n_group2
      )

    tukey_rows <- dplyr::bind_cols(tukey_rows_core, make_stats_context(
      n_rows = nrow(tukey_rows_core),
      sided = "two-sided",
      covariates_tested = covariates_tested,
      assumptions_or_corrections = assumptions_or_corrections,
      experimental_unit = experimental_unit,
    ...
    dplyr::bind_rows(anova_row, tukey_rows)
  }))
}

make_welch_bh_table <- function(data, value_col, group_col, metric = value_col,
                                experimental_unit = "independent bioprinted array",
                                analysis_level = "tracked cell trajectory",
                                hierarchical_level = "Tracked cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized per trajectory before testing.",
                                assumptions_or_corrections = "Welch ANOVA and Welch two-sample t-tests were used to avoid assuming equal variances; Benjamini-Hochberg adjustment was used for multiple pairwise comparisons.",
                                covariates_tested = default_covariates) {
  ...
  form <- stats::reformulate(group_col, response = value_col)
  welch <- stats::oneway.test(form, data = d, var.equal = FALSE)
  eta2 <- eta_squared_manual(d, value_col, group_col)
  ...
  global_row <- tibble::tibble(
    ...
    test = "Welch ANOVA",
    statistic = "F",
    statistic_value = unname(welch$statistic),
    ...
    p_adjust_method = "none",
    p_adjusted = NA_real_,
    significance = p_to_stars(welch$p.value),
    effect_size_type = "eta_squared_descriptive",
    effect_size = eta2,
    effect_size_ci_low = NA_real_,
    effect_size_ci_high = NA_real_,
    effect_size_calculation = "Descriptive eta-squared calculated as between-group sum of squares divided by total sum of squares; Welch ANOVA was used for the P value.",
    n_total = nrow(d),
    n_group1 = NA_integer_,
    n_group2 = NA_integer_
  ) %>%
  ...
  pairwise_rows_core <- dplyr::bind_rows(lapply(combn(groups, 2, simplify = FALSE), function(g) {

    sub <- d %>%
      dplyr::filter(.data[[group_col]] %in% g) %>%
      dplyr::mutate(
        "{group_col}" := factor(.data[[group_col]], levels = g)
      )

    tt <- stats::t.test(form, data = sub, var.equal = FALSE, conf.level = 0.95)
    eff <- hedges_g_manual(sub, value_col, group_col, g[1], g[2])

    tibble::tibble(
      metric = metric,
      analysis_group = NA_character_,
      comparison = paste(g[1], g[2], sep = " - "),
      test = "Welch two-sample t-test",
      statistic = "t",
      statistic_value = unname(tt$statistic),
      ...
      n_total = nrow(d),
      n_group1 = sum(sub[[group_col]] == g[1]),
      n_group2 = sum(sub[[group_col]] == g[2])
    ) %>%
      dplyr::bind_cols(eff)
  })) %>%
    dplyr::mutate(
  ...
  pairwise_rows <- dplyr::bind_cols(pairwise_rows_core, make_stats_context(
    n_rows = nrow(pairwise_rows_core),
    sided = "two-sided",
    covariates_tested = covariates_tested,
    assumptions_or_corrections = assumptions_or_corrections,
    experimental_unit = experimental_unit,
  ...
  dplyr::bind_rows(global_row, pairwise_rows)
}

make_alpha_table <- function(data, group_cols, tau_col, tasd_col = "tasd",
                             tau_range = c(50, 500),
                             metric = "Anomalous diffusion exponent alpha",
                             experimental_unit = "tracked cell trajectories summarized by group",
                             analysis_level = "group-level TASD curve",
                             hierarchical_level = "TASD was calculated from repeated time-lapse measurements within trajectories and summarized by group and lag time before regression.",
                             assumptions_or_corrections = "Alpha was estimated by linear regression of log(TASD) against log(lag time) over the stated lag-time window.",
                             covariates_tested = default_covariates) {

  required <- c(group_cols, tau_col, tasd_col)

  if (!all(required %in% names(data))) {
    return(tibble::tibble(
      note = paste("Missing columns:", paste(setdiff(required, names(data)), collapse = ", "))
    ))
  }
  ...
  out <- data %>%
    ...
    dplyr::group_modify(~{
      ...
      if (nrow(.x) < 3) {
        return(tibble::tibble(
          ...
          coefficient = "alpha; slope of log(TASD) versus log(lag time)",
          estimate = NA_real_,
          estimate_type = "regression slope",
          ci_low = NA_real_,
          ci_high = NA_real_,
          statistic = "t",
          statistic_value = NA_real_,
          ...
          effect_size_ci_low = NA_real_,
          effect_size_ci_high = NA_real_,
          effect_size_calculation = "R-squared from linear regression of log(TASD) on log(lag time).",
          n_points = nrow(.x)
        ))
      }

      fit <- lm(log_tasd ~ log_tau, data = .x)
      ci <- suppressMessages(confint(fit))
      tid <- broom::tidy(fit) %>% dplyr::filter(term == "log_tau")
      ...
      tibble::tibble(
        ...
        test = "linear regression",
        coefficient = "alpha; slope of log(TASD) versus log(lag time)",
        estimate = tid$estimate[1],
        estimate_type = "regression slope",
        ci_low = ci["log_tau", 1],
        ci_high = ci["log_tau", 2],
        statistic = "t",
        statistic_value = tid$statistic[1],
        df = fit$df.residual,
        p_value = tid$p.value[1],
        r_squared = gla$r.squared[1],
        effect_size_type = "r_squared",
        effect_size = gla$r.squared[1],
        effect_size_ci_low = NA_real_,
        effect_size_ci_high = NA_real_,
        effect_size_calculation = "R-squared from linear regression of log(TASD) on log(lag time).",
        n_points = nrow(.x)
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      sided = "two-sided test of regression slope",
      covariates_tested = covariates_tested,
      assumptions_or_corrections = assumptions_or_corrections,
      experimental_unit = experimental_unit,
      analysis_level = analysis_level,
...
make_source_readme <- function() {
  tibble::tibble(
    reporting_summary_item = c(
      "Purpose",
      "Exact sample size",
      "Statistical tests and sidedness",
      "Covariates",
      "Assumptions and corrections",
      "Central tendency and variation",
      "Null hypothesis test statistics",
      "Hierarchical and complex designs",
      "Effect sizes",
      "Panels without null hypothesis tests"
    ),
    how_addressed_in_workbook = c(
      "Source data workbook for Figure 4. Sheets contain raw plotted values, summary statistics, test output and reporting metadata where applicable.",
      "Summary sheets report n as a discrete number and include n_unit. Statistics sheets report n_total and pairwise n_group1/n_group2 where applicable.",
      "Statistics sheets report the test used and the sided column. ANOVA/Welch ANOVA rows are omnibus F tests; pairwise t/Tukey rows are two-sided.",
      default_covariates,
      "Corrections are reported in p_adjust_method. Arrangement panels use Tukey correction. Composition panels use Benjamini-Hochberg correction after Welch pairwise tests. No formal normality test was used as a decision rule for Figure 4.",
      "Summary sheets report mean, SD, SEM, median, quartiles, min and max. Statistics sheets report mean differences and confidence intervals where applicable.",
      "Statistics sheets report F, t or regression slope statistics, degrees of freedom, exact P values, adjusted P values and confidence intervals where applicable.",
      "For Fig. 4C, 4D, 4J and 4K, each plotted point is a tracked cell trajectory nested within independent bioprinted arrays. For Fig. 4F-H, analyses are at the independent array level. Repeated time-lapse measurements were summarized before statistical testing where applicable.",
      "Global ANOVA rows report eta-squared. Welch ANOVA rows report descriptive eta-squared. Pairwise comparisons report Hedges' g. Alpha regression rows report R-squared. The effect_size_calculation column states how each was calculated.",
      "Trajectory visualizations, TASD curves, COM time courses, COM vectors and nearest-neighbor time courses are exported as source/plot data. They are treated as descriptive unless a statistics sheet is included."
    )
  )
}
```
...
## Source Data Export
```{r}
# Arrangement source data: Fig. 4B-H
fig4_wb <- load_or_create_wb()
add_or_replace_sheet(fig4_wb, "README_reporting", make_source_readme())

# Fig. 4B, trajectories
if (exists("Combined_Positions")) {
  fig4b_tracks <- Combined_Positions %>%
    dplyr::mutate(CellTypeID = dplyr::recode(CellTypeID, "TNBC" = "MDAMB231")) %>%
    dplyr::filter(
      Group %in% c("Segregated", "Clustered", "Mixed"),
      CellTypeID %in% c("HUVEC", "MDAMB231")
    ) %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "Time", "X_Position", "Y_Position")) %>%
    arrange_existing(c("Group", "SampleID", "CellTypeID", "UniqueID", "Time"))

  add_or_replace_sheet(fig4_wb, "Fig4B_tracks", fig4b_tracks)
}

# Fig. 4C, total distance traveled
if (exists("df_len")) {
  fig4c_raw <- df_len %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "TrackLength")) %>%
    arrange_existing(c("CellTypeID", "Group", "SampleID", "UniqueID"))

  fig4c_summary <- make_summary(
    data = df_len,
    group_cols = c("CellTypeID", "Group"),
    value_col = "TrackLength",
    units = "µm",
    n_unit = "tracked cell trajectories"
  )

  fig4c_stats <- make_anova_tukey_table(
    data = df_len,
    value_col = "TrackLength",
    group_col = "Group",
    split_col = "CellTypeID",
    metric = "Total distance traveled",
    experimental_unit = "independent bioprinted array",
    analysis_level = "tracked cell trajectory",
    hierarchical_level = "Tracked cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized into total distance traveled per trajectory before statistical testing."
  )

  add_or_replace_sheet(fig4_wb, "Fig4C_raw", fig4c_raw)
  add_or_replace_sheet(fig4_wb, "Fig4C_summary", fig4c_summary)
  add_or_replace_sheet(fig4_wb, "Fig4C_stats", fig4c_stats)
}

# Fig. 4D, net displacement
if (exists("disp_df")) {
  fig4d_raw <- disp_df %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "Displacement_um")) %>%
    arrange_existing(c("CellTypeID", "Group", "SampleID", "UniqueID"))

  fig4d_summary <- make_summary(
    data = disp_df,
    group_cols = c("CellTypeID", "Group"),
    value_col = "Displacement_um",
    units = "µm",
    n_unit = "tracked cell trajectories"
  )

  fig4d_stats <- make_anova_tukey_table(
    data = disp_df,
    value_col = "Displacement_um",
    group_col = "Group",
    split_col = "CellTypeID",
    metric = "Net displacement",
    experimental_unit = "independent bioprinted array",
    analysis_level = "tracked cell trajectory",
    hierarchical_level = "Tracked cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized into net displacement per trajectory before statistical testing."
  )

  add_or_replace_sheet(fig4_wb, "Fig4D_raw", fig4d_raw)
  add_or_replace_sheet(fig4_wb, "Fig4D_summary", fig4d_summary)
  add_or_replace_sheet(fig4_wb, "Fig4D_stats", fig4d_stats)
}

# Fig. 4E, TASD and alpha
if (exists("all_sum_trim")) {
  fig4e_plot <- all_sum_trim %>%
    dplyr::mutate(
      tau_hr = if ("tau_min" %in% names(.)) tau_min / 60 else NA_real_,
      n_unit = "tracked cell trajectories contributing at each lag time",
      measurement_unit = "MSD: µm^2; lag time: minutes/hours"
    ) %>%
    select_existing(c("CellTypeID", "Group", "tau_frames", "tau_min", "tau_hr", "tasd", "sem", "n_tracks", "n_unit", "measurement_unit")) %>%
    arrange_existing(c("CellTypeID", "Group", "tau_frames"))

  fig4e_alpha <- make_alpha_table(
    data = all_sum_trim,
    group_cols = c("CellTypeID", "Group"),
    tau_col = "tau_min",
    tasd_col = "tasd",
  ...
  add_or_replace_sheet(fig4_wb, "Fig4E_plotdata", fig4e_plot)
  add_or_replace_sheet(fig4_wb, "Fig4E_alpha", fig4e_alpha)
}

# Fig. 4F, HUVEC center-of-mass time course
if (exists("com_centered")) {
  fig4f_raw <- com_centered %>%
    dplyr::mutate(
      experimental_unit = "independent bioprinted array",
      analysis_level = "array-level center-of-mass displacement over time",
      measurement_unit = "µm"
    ) %>%
    select_existing(c("SampleID", "Group", "frame", "Time_hours", "com_x", "com_y", "dx", "dy", "experimental_unit", "analysis_level", "measurement_unit")) %>%
    arrange_existing(c("Group", "SampleID", "Time_hours", "frame"))

  add_or_replace_sheet(fig4_wb, "Fig4F_raw", fig4f_raw)
}

if (exists("com_summary")) {
  fig4f_summary <- com_summary %>%
    dplyr::mutate(
      n_unit = "independent bioprinted arrays contributing at each time point",
      measurement_unit = "µm",
      assumptions_or_corrections = "Descriptive time-course plot; no null hypothesis test was performed for this panel."
    ) %>%
    arrange_existing(c("Group", "Time_hours"))

  add_or_replace_sheet(fig4_wb, "Fig4F_summary", fig4f_summary)
}

# Fig. 4G, time-averaged HUVEC center-of-mass displacement
if (exists("df_box")) {
  fig4g_raw <- df_box %>%
    select_existing(c("SampleID", "Group", "Component", "Value")) %>%
    arrange_existing(c("Component", "Group", "SampleID"))

  fig4g_summary <- make_summary(
    data = df_box,
    group_cols = c("Component", "Group"),
    value_col = "Value",
    units = "µm",
    n_unit = "independent bioprinted arrays"
  )

  fig4g_stats <- make_anova_tukey_table(
    data = df_box,
    value_col = "Value",
    group_col = "Group",
    split_col = "Component",
    metric = "Time-averaged HUVEC COM displacement",
    experimental_unit = "independent bioprinted array",
    analysis_level = "array-level time-averaged center-of-mass displacement",
    hierarchical_level = "Repeated time-lapse measurements were summarized into one time-averaged COM displacement value per independent bioprinted array before statistical testing.",
    assumptions_or_corrections = "One-way ANOVA was used for predefined spatial-arrangement comparisons; Tukey adjustment was used for multiple pairwise comparisons."
  )

  add_or_replace_sheet(fig4_wb, "Fig4G_raw", fig4g_raw)
  add_or_replace_sheet(fig4_wb, "Fig4G_summary", fig4g_summary)
  add_or_replace_sheet(fig4_wb, "Fig4G_stats", fig4g_stats)
}

# Fig. 4H, HUVEC center-of-mass displacement vectors
if (exists("repl_df")) {
  fig4h_raw <- repl_df %>%
    dplyr::mutate(
      experimental_unit = "independent bioprinted array",
      analysis_level = "array-level endpoint COM displacement vector",
      measurement_unit = "µm for dx_end/dy_end; degrees for bearing_deg",
      assumptions_or_corrections = "Descriptive vector visualization; no null hypothesis test was performed for this panel."
    ) %>%
    select_existing(c("SampleID", "Group", "CellTypeID", "Time_end", "dx_end", "dy_end", "bearing_deg", "experimental_unit", "analysis_level", "measurement_unit", "assumptions_or_corrections")) %>%
    arrange_existing(c("Group", "SampleID"))

  add_or_replace_sheet(fig4_wb, "Fig4H_vectors", fig4h_raw)
}

openxlsx::saveWorkbook(fig4_wb, fig4_source_file, overwrite = TRUE)
message("Saved arrangement source data: ", fig4_source_file)
```
...
## Source Data Export
```{r}
# Composition source data: Fig. 4I-M
# This appends to Nature_Source_Data/Figure4_SourceData.xlsx
fig4_wb <- load_or_create_wb()

# Fig. 4I, composition trajectories
if (exists("Combined_Positions")) {
  fig4i_tracks <- Combined_Positions %>%
    dplyr::filter(Group %in% c("TNBC", "TNBC+Myoep", "TNBC+FB", "TNBC+Mac")) %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "Time", "X_Position", "Y_Position")) %>%
    arrange_existing(c("Group", "SampleID", "CellTypeID", "UniqueID", "Time"))

  add_or_replace_sheet(fig4_wb, "Fig4I_tracks", fig4i_tracks)
}

# Fig. 4J, TNBC total distance traveled by composition
if (exists("df_len") && "TrackLength" %in% names(df_len)) {
  fig4j_raw <- df_len %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "TrackLength")) %>%
    arrange_existing(c("Group", "SampleID", "UniqueID"))

  fig4j_summary <- make_summary(
    data = df_len,
    group_cols = c("Group"),
    value_col = "TrackLength",
    units = "µm",
    n_unit = "tracked TNBC cell trajectories"
  )

  fig4j_stats <- make_welch_bh_table(
    data = df_len,
    value_col = "TrackLength",
    group_col = "Group",
    metric = "TNBC total distance traveled by cellular composition",
    experimental_unit = "independent bioprinted array",
    analysis_level = "tracked TNBC cell trajectory",
    hierarchical_level = "Tracked TNBC cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized into total distance traveled per trajectory before statistical testing."
  )

  add_or_replace_sheet(fig4_wb, "Fig4J_raw", fig4j_raw)
  add_or_replace_sheet(fig4_wb, "Fig4J_summary", fig4j_summary)
  add_or_replace_sheet(fig4_wb, "Fig4J_stats", fig4j_stats)
}

# Fig. 4K, TNBC distance traveled normalized by observation time
if (exists("df_speed") && "Dist_um_per_frame" %in% names(df_speed)) {
  fig4k_raw <- df_speed %>%
    select_existing(c("SampleID", "UniqueID", "CellTypeID", "Group", "TrackLength", "n_frames_visible", "Dist_um_per_frame")) %>%
    arrange_existing(c("Group", "SampleID", "UniqueID"))

  fig4k_summary <- make_summary(
    data = df_speed,
    group_cols = c("Group"),
    value_col = "Dist_um_per_frame",
    units = "µm per frame",
    n_unit = "tracked TNBC cell trajectories"
  )

  fig4k_stats <- make_welch_bh_table(
    data = df_speed,
    value_col = "Dist_um_per_frame",
    group_col = "Group",
    metric = "TNBC distance traveled normalized by observation time",
    experimental_unit = "independent bioprinted array",
    analysis_level = "tracked TNBC cell trajectory",
    hierarchical_level = "Tracked TNBC cell trajectories are nested within independent bioprinted arrays; repeated time-lapse measurements were summarized into normalized distance traveled per trajectory before statistical testing."
  )

  add_or_replace_sheet(fig4_wb, "Fig4K_raw", fig4k_raw)
  add_or_replace_sheet(fig4_wb, "Fig4K_summary", fig4k_summary)
  add_or_replace_sheet(fig4_wb, "Fig4K_stats", fig4k_stats)
}

# Fig. 4L, TNBC TASD and alpha by composition
if (exists("mdamb_sum_trim")) {
  fig4l_alpha_input <- mdamb_sum_trim
  if ("tau_hr" %in% names(fig4l_alpha_input)) {
    fig4l_alpha_input <- fig4l_alpha_input %>% dplyr::mutate(tau_min = tau_hr * 60)
  }

  fig4l_plot <- fig4l_alpha_input %>%
    dplyr::mutate(
      n_unit = "tracked TNBC cell trajectories contributing at each lag time",
      measurement_unit = "MSD: µm^2; lag time: hours/minutes"
    ) %>%
    select_existing(c("CellTypeID", "Group", "tau_frames", "tau_hr", "tau_min", "tasd", "sem", "n_tracks", "n_unit", "measurement_unit")) %>%
    arrange_existing(c("Group", "tau_frames"))

  fig4l_alpha <- make_alpha_table(
    data = fig4l_alpha_input,
    group_cols = c("CellTypeID", "Group"),
    tau_col = "tau_min",
    tasd_col = "tasd",
    tau_range = c(50, 500),
  ...
  add_or_replace_sheet(fig4_wb, "Fig4L_plotdata", fig4l_plot)
  add_or_replace_sheet(fig4_wb, "Fig4L_alpha", fig4l_alpha)
}

# Fig. 4M, mean distance to nearest non-TNBC neighbor over time
if (exists("nn_mdamb")) {
  fig4m_raw <- nn_mdamb %>%
    dplyr::mutate(
      experimental_unit = "independent bioprinted array",
      analysis_level = "TNBC cell nearest-neighbor distance over time",
      measurement_unit = "µm",
      assumptions_or_corrections = "Descriptive nearest-neighbor time-course plot; no null hypothesis test was performed for this panel."
    ) %>%
    select_existing(c("SampleID", "Group", "Time_frame", "Time_hr", "MDA_ID", "MDA_X", "MDA_Y", "NN_Dist_um", "NN_CellTypeID", "NN_UniqueID", "experimental_unit", "analysis_level", "measurement_unit", "assumptions_or_corrections")) %>%
    arrange_existing(c("Group", "SampleID", "MDA_ID", "Time_hr"))

  add_or_replace_sheet(fig4_wb, "Fig4M_raw", fig4m_raw)
}

if (exists("nn_time_summary")) {
  fig4m_summary <- nn_time_summary %>%
    dplyr::mutate(
      n_unit = "TNBC cell nearest-neighbor measurements contributing at each time point",
      measurement_unit = "µm",
      assumptions_or_corrections = "Descriptive nearest-neighbor time-course plot; no null hypothesis test was performed for this panel."
    ) %>%
    arrange_existing(c("Group", "Time_hr"))

  add_or_replace_sheet(fig4_wb, "Fig4M_summary", fig4m_summary)
}

openxlsx::saveWorkbook(fig4_wb, fig4_source_file, overwrite = TRUE)
message("Saved complete Figure 4 source data: ", fig4_source_file)
```
